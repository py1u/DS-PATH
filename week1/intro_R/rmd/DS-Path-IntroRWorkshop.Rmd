...
#get eruption time
erupt_time <- faithful$eruptions
erupt_time

#convert eruption time
convertTime <- function(erupt_time){
  erupt_int <- as.integer(erupt_time)
  return(erupt_int)
}

convertTime(erupt_time)
```
...
#warning displays since they are vectors of not same length, but 10 gets recycled for value 20 from A
a + b
...
#no error since they are even to even length vectors, same with odd to odd
a + c
```
...
calculate_factorial <- function(n){
  ...
  if(is.integer(n) && n %% 1 == 0  &&n >= 0){
    ...
    print("Error: input print non-negative number")
    print(NULL)
  }
...
if(x>y){
  cat(x,"greater than", y)
}else{
  cat(x, "is than or equal to", y)
}
```
